...
Importación de Colecciones: Crea 3 colecciones (Listings, Reviews, Calendar) e importa su archivo CSV correspondiente a cada una. (Nota: El archivo Calendar tiene un volumen superior a los 10 millones de registros, por lo que la importación puede tomar varios minutos).
...
Fase 0: Análisis Exploratorio (Opcional) Para revisar los hallazgos iniciales en el entorno interactivo:

notebooks/exploracion_airbnb.ipynb

Fase 1: Extracción Establece la conexión con MongoDB, consulta las colecciones y las convierte en DataFrames estructurados.

python src/extraccion.py
Fase 2: Transformación Recibe los datos crudos, ejecuta la limpieza, maneja valores nulos y normaliza los formatos.

python src/transformacion.py
Fase 3: Carga Consolida la información limpia y la exporta al destino final (base de datos SQLite y reporte en Excel).
